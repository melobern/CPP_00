...
static bool is_full_digits(std::string str) {
    ...
    while (str[x]) {
        if (x > 9) {
            std::cout << "Sorry, the phone number must contain 8 or 10 digits.";
            std::cout << std::endl;
            return (0);
        }
        if (!std::isdigit(str[x])) {
            std::cout << "Sorry, this field must contain digits only.";
            std::cout << std::endl;
            return (0);
        }
        x++;
...
static bool is_full_letters(std::string str, std::string label) {
    ...
    while (str[x]) {
        if (!std::isalpha(str[x]) && !std::isspace(str[x]) && str[x] != '-') {
            std::cout << "Sorry, " << label << " must contain letters only.";
            std::cout << std::endl;
            return (0);
        }
        x++;
...
void    Contact::setPhone(void) {
    ...
    while (true) {
        ...
        if (!(input_len == 8 || input_len == 10)) {
            std::cout << "Sorry, the phone must contains 8 or 10 digits.";
            std::cout << std::endl;
            continue;
        }
        this->m_phone = input;
